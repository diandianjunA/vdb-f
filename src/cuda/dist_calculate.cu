...
std::vector<float> DistanceCalculator::computeDistances(std::vector<char *> &data,
                                    cudaStream_t stream) {
...
float DistanceCalculator::computeSingleDistanceAsync(char *d_vec, cudaStream_t stream) {
    ...
    cudaDeviceSynchronize();
    float result;
    cudaMemcpyAsync(&result, d_result, sizeof(float),
